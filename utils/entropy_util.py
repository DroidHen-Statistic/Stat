...
__all__=['entropy', 'mutual_information', 'entropy_gaussian', 'calc_ent', 'calc_condition_ent', 'calc_ent_grap']
...
def calc_ent(x):
    """
        calculate shanno ent of x, dtype = int
    """
...
def calc_ent_grap(x,y):
    """
        calculate ent grap I(x,y) = H(x) - H(x|y) = H(y) - H(y|x), dtype = int
    """
...
def entropy(X, k=1):
    ...
    X : array-like, shape (n_samples, n_features)
        The data the entropy of which is computed, can not be int
    k : int, optional
...
def my_test():
    ...
    info = mutual_information_2d(X.T[0], y)
    print(info)
    # info = mutual_info_classif(X, y)
    # print(info)
    # h_X = [ calc_ent(vector) for vector in X.T]
    # h_y = calc_ent(y)
    # for pos, ent in enumerate(h_X):
    #     info = calc_ent_grap(X[:,pos], y)
    #     print(info)
        # n_info = 2 * info / (ent + h_y)
        # print(n_info)
