...
def train(x, y):
    ...
    print(np.mean(tmp))
    odds_pay = np.mean(np.array(x[-31:])[:,-5:], axis = 0)
    print(odds_pay)
    odds_no_pay = np.mean(np.array(x[:-31])[:,-5:], axis = 0)

    odds_pay_var = np.var(np.array(x[-31:])[:,-5:], axis = 0)
    print(odds_pay_var)
    odds_no_pay_var = np.var(np.array(x[:-31])[:,-5:], axis = 0)
    print(odds_no_pay_var)

    plt.plot(odds_pay_var, label = 'pay')
    plt.plot(odds_no_pay_var,label = 'no_pay')
...
if __name__ == '__main__':
    ...
    train(uid_2_vectors["1675766"][0], uid_2_vectors["1675766"][1])
